    ...
    def __getitem__(self, index):
        ...
        for img_file in image_files:
            ...
            if self.load_ann:
                ...
                main_crop,dims_main = random_crop(img)
                annot = torch.from_numpy(annot[dims_main[1]:dims_main[1]+dims_main[2],dims_main[0]:dims_main[0]+dims_main[2]])
                annot = F.interpolate(annot.unsqueeze(0).unsqueeze(0),size=(FLAGS.image_size,FLAGS.image_size),mode='nearest')
                annot = F.one_hot(annot.long().squeeze(1), 460).movedim(3,1)
                annots.append(annot)
            else:
