...
class VisionParser(nn.Module):
    ...
    def assign_nearest_clusters(self, feature_maps):
        ...
        sims = sims.reshape(FLAGS.batch_size, FLAGS.image_size//8, FLAGS.image_size//8, FLAGS.num_prototypes).movedim(3,1) # B,num_prototypes,img_size/8,img_size/8
    ...
    def spatial_map_cluster_assign(self, feature_maps, masks, crop_dims):
        ...
        proto_features_a = proto_features_a.sum(dim=(3,4)) / (mask_a_count[:,valid_protos_a].unsqueeze(2) + 0.001) # B,num_valid_a,c
        ...
        sim_features_b = sim_features_b.sum(dim=(3,4)) / (mask_b_count[:,valid_protos_a].unsqueeze(2) + 0.001) # B,num_valid_a,c
        ...
        proto_features_b = proto_features_b.sum(dim=(3,4)) / (mask_b_count[:,valid_protos_b].unsqueeze(2) + 0.001) # B,num_valid_b,c
        ...
        sim_features_a = sim_features_a.sum(dim=(3,4)) / (mask_a_count[:,valid_protos_b].unsqueeze(2) + 0.001) # B,num_valid_b,c
